...
/*
1::
	; oZoom.BackUphdcMemory := oZoom.hdcMemory
	oZoom.CrophdcMemory := CreateCompatibleDC()
	
	SysGet, VirtualScreenWidth, 78
	SysGet, VirtualScreenHeight, 79 
	
	hBM := DllCall("Gdi32.Dll\CreateCompatibleBitmap", "Ptr", oZoom.hdcMemory, "Int", VirtualScreenWidth, "Int", VirtualScreenHeight)
	DllCall("Gdi32.Dll\SelectObject", "Ptr", oZoom.CrophdcMemory, "Ptr", hBM) 
	StretchBlt(oZoom.CrophdcMemory, 0, 0, VirtualScreenWidth, VirtualScreenHeight, oZoom.hdcMemory, 0, 0, VirtualScreenWidth, VirtualScreenHeight)
	DllCall("gdiplus\GdipCreateBitmapFromHBITMAP", "UPtr", hbm, "UPtr", 0, "UPtr*", pBitmap) 
	DllCall("gdiplus\GdipCreateFromHDC", "UPtr", oZoom.CrophdcMemory, "UPtr*", G) 
	GaussianBlur(pBitmap, 1, 1)
	DrawImage(G, pBitmap, 0, 0, 111, 111)
	oZoom.VirtualScreenWidth := VirtualScreenWidth, oZoom.VirtualScreenHeight := VirtualScreenHeight
	; UpdateLayeredWindow(oZoom.hLW, oZoom.CrophdcMemory, oZoom.LWWidth, oZoom.LWHeight)
...
GaussianBlur(ByRef pBitmap, radius, opacity := 1) {
	...
	clone := CloneBitmapArea(pBitmap, 0, 0, width, height)
	E1 := LockBits(pBitmap, 0, 0, width, height, Stride1, Scan01, BitmapData1)
	E2 := LockBits(clone, 0, 0, width, height, Stride2, Scan02, BitmapData2)
	...
	UnlockBits(pBitmap, BitmapData1)
	UnlockBits(clone, BitmapData2) 
	DllCall("gdiplus\GdipDisposeImage", "UPtr", clone)
	return value
}
...
GetImageDimensions(pBitmap, ByRef Width, ByRef Height) {
   ...
   Width := 0, Height := 0
   E := GetImageDimension(pBitmap, Width, Height)
   Width := Round(Width)
   Height := Round(Height)
   return E
}

GetImageDimension(pBitmap, ByRef w, ByRef h) {
   Ptr := A_PtrSize ? "UPtr" : "UInt"
   return DllCall("gdiplus\GdipGetImageDimension", Ptr, pBitmap, "float*", w, "float*", h)
} 

CloneBitmapArea(pBitmap, x:="", y:="", w:=0, h:=0, PixelFormat:=0, KeepPixelFormat:=0) {
...
; the resulted pBitmap is erroneuous / defective.
   ...
   If (KeepPixelFormat=1)
      PixelFormat := GetImagePixelFormat(pBitmap, 1)
...
GetImagePixelFormat(pBitmap, mode:=0) { 
...
GraphicsFromImage(pBitmap, InterpolationMode:="", SmoothingMode:="", PageUnit:="", CompositingQuality:="") {
   pGraphics := 0
   ...
   If pGraphics
   {
      If (InterpolationMode!="")
         SetInterpolationMode(pGraphics, InterpolationMode)
      If (SmoothingMode!="")
         SetInterpolationMode(pGraphics, SmoothingMode)
      If (PageUnit!="")
         SetPageUnit(pGraphics, PageUnit)
      If (CompositingQuality!="")
         SetInterpolationMode(pGraphics, CompositingQuality)
   }
   return pGraphics
}

SetPageUnit(pGraphics, Unit) { 
   Ptr := A_PtrSize ? "UPtr" : "UInt"
   return DllCall("gdiplus\GdipSetPageUnit", Ptr, pGraphics, "int", Unit)
}

CreateBitmap(Width, Height, PixelFormat:=0, Stride:=0, Scan0:=0) { 
   pBitmap := 0
   If !PixelFormat
      PixelFormat := 0x26200A  ; 32-ARGB
