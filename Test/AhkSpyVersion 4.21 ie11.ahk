...
Global AhkSpyVersion := 4.21 ie11
...
; , _BP1 := "<span contenteditable='false' oncontextmenu='return false' class='BB'>" _BT1 "style='color: #" ColorParam "' lang='pre' ", _BP2 := "</span></span>"
...
;; , _BP1 := "<span class='button' unselectable='on' oncontextmenu='return false' onmouseleave='OnButtonOut (this)' onmousedown='OnButtonDown (this)' "
	;; . "onmouseup='OnButtonUp (this)' onmouseover='OnButtonOver (this)' contenteditable='false' style='color: #" ColorParam "' lang='pre' ", _BP2 := "</span>"
...
, _StIf := "    <span style='color: #f0f0f0'>&#9642</span>    <span class='faded_color' lang='MS:' style='color: #C0C0C0'>"
...
Spot_Win(NotHTML = 0) {
	...
	WinGet, WinExStyle, ExStyle, ahk_id %WinID% 
	{
		...
		If WinTransparent !=
			TransparentStr := _BP1 "id='set_button_Transparent'>Transparent:</span>" _BP2 "  <span id='get_win_Transparent' lang='MS:'>"  WinTransparent "</span>"     ;;  _INPHK "id='edithotkey' value='" WinTransparent "'>"
	
		WinGet, WinTransColor, TransColor, ahk_id %WinID%
		If WinTransColor !=
			TransColorStr := _BP1 "id='set_button_TransColor'>TransColor:</span>" _BP2 "  <span id='get_win_TransColor' lang='MS:'>"  WinTransColor "</span>"
	
		OwnedId := DllCall("GetWindow", "Ptr", WinID, UInt, 4, "Ptr")
		If OwnedId
			OwnedIdStr := "<span class='param'>Owned Id:</span> <span lang='MS:'>" Format("0x{:x}", OwnedId) "</span>"
	...
	if sb_fields.maxindex()
	{
		...
		for k, v in sb_fields
			SBText .= "<span class='param'>(" k "):</span> <span lang='MS:' id='sb_field_" A_Index "'>" TransformHTML(v "") "</span>`n"
		If SBText !=
			SBText := _T1 " id='__StatusBarText'> ( StatusBarText ) </span>" _BT1 " id='copy_sbtext' lang='" sb_max "'> copy " _BT2 _T2 _PRE1 "<span>" SBText "</span></span>" _PRE2
	}
	...
	If ViewStrPos
		ViewStrPos1 := _DP "<span lang='MS:'>" WinX ", " WinY ", " WinX2 ", " WinY2 "</span>" _DP "<span lang='MS:'>" WinX ", " WinY ", " WinWidth ", " WinHeight "</span>"
	...
	If (hParent := DllCall("GetAncestor", "Ptr", WinID, Uint, 1)) && (hParent != WinID)
	{
		...
		_ParentWindow := "`n<span class='param'>Parent window:</span>  <span lang='MS:'>" ParentClass "</span>" _DP " <span lang='MS:'>" ParentProcessName "</span>" _DP "<span lang='MS:'>" Format("0x{:x}", hParent) "</span>"
	}
...
HTML_Win:
	...
	%_PRE1%<span id='wintitle1' lang='MS:'>%WinTitle%</span>%_PRE2%
	%_T1% id='__Class'> ( Class ) </span>%_T2%
	%_PRE1%<span id='wintitle2'><span class='param' id='wintitle2_' lang='MS:S'>ahk_class </span><span lang='MS:'>%WinClass%</span></span>%_PRE2%
	%_T1% id='__ProcessName'> ( ProcessName ) </span>%_BT1% id='copy_alltitle'> copy titles %_BT2%%_T2%
	%_PRE1%<span id='wintitle3'><span class='param' lang='MS:S' id='wintitle3_'>ahk_exe </span><span lang='MS:'>%WinProcessName%</span></span>%_PRE2%
	%_T1% id='__ProcessPath'> ( ProcessPath ) </span>%_BT1% id='infolder'> in folder %_BT2%%_DB%%_BT1% id='paste_process_path'> paste %_BT2%%_T2%
	%_PRE1%<span><span class='param' lang='MS:S'>ahk_exe </span><span id='copy_processpath' lang='MS:'>%WinProcessPath%</span></span>%_PRE2%
	...
	%_PRE1%<span id='c_command_line' lang='MS:'>%ComLine%</span>%_PRE2%
	%_T1% id='__Position'> ( Position ) </span>%_T2%
	%_PRE1%%_BP1% id='set_button_pos'>Pos:%_BP2%  <span lang='MS:'>x%WinX% y%WinY%</span>%_DP%<span lang='MS:'>x&sup2;%WinX2% y&sup2;%WinY2%</span>%_DP%%_BP1% id='set_button_pos'>Size:%_BP2%  <span lang='MS:'>w%WinWidth% h%WinHeight%</span>%ViewStrPos1%
	<span class='param'>Client area size:</span>  <span lang='MS:'>w%caW% h%caH%</span>%_DP%<span class='param'>left</span> %caX% <span class='param'>top</span> %caY% <span class='param'>right</span> %caWinRight% <span class='param'>bottom</span> %caWinBottom%%_PRE2%
	%_T1% id='__Other'> ( Other ) </span>%_BT1% id='flash_window'> flash %_BT2%%_ButWindow_Detective%%_T2%
	%_PRE1%<span class='param' lang='MS:N'>PID:</span>  <span lang='MS:'>%WinPID%</span>%_DP%%ProcessBitSize%%IsAdmin%<span class='param'>Window count:</span> %WinCountProcess%%_DP%%_BB1% id='process_close'> process close %_BB2%
	<span class='param' lang='MS:N'>HWND:</span>  <span lang='MS:'>%WinID%</span>%_DP%%_BB1% id='window_show_hide'> show / hide %_BB2%%_DP%%_BB1% id='win_close'> destroy %_BB2%%_DP%<span class='param'>Control count:</span>  %CountControl%%IsWindowUnicodeStr%%_ParentWindow%%EX1Str%%CLSID%
	<span class='param'>Style:  </span><span id='w_Style' lang='MS:'>%WinStyle%</span>%_DP%<span class='param'>ExStyle:  </span><span id='w_ExStyle' lang='MS:'>%WinExStyle%</span>%ButtonStyle_%%_PRE2%
...
Spot_Control(NotHTML = 0) {
	...
	cc = %_DP%%_BB1% lang='x%RWinX% y%RWinY%' id='control_click'> control click %_BB2% 
	WithRespectWin := cc "`n" _BP1 " id='set_pos'>Relative window:" _BP2 "  <span lang='MS:'>"
		. Round(RWinX / WinW, 4) ", " Round(RWinY / WinH, 4) "</span>  <span class='param'>for</span> <span lang='MS:'>w" WinW " h" WinH "</span>" _DP
	WithRespectClient := _BP1 " id='set_pos'>Relative client:" _BP2 "  <span lang='MS:'>" Round(MXC / caW, 4) ", " Round(MYC / caH, 4)
		. "</span>  <span class='param'>for</span> <span lang='MS:'>w" caW " h" caH "</span>"
	...
	If ControlID
	{
		...
		{
			WinGetClass, ParentClass, ahk_id %hParent%
			_ParentControl := _DP "<span class='param'>Parent control:</span>  <span lang='MS:'>" ParentClass "</span>" _DP "<span lang='MS:'>" Format("0x{:x}", hParent) "</span>"
		}
		
		If ViewStrPos
			ViewStrPos1 := _DP "<span lang='MS:'>" CtrlX ", " CtrlY ", " CtrlX2 ", " CtrlY2 "</span>" _DP "<span lang='MS:'>" CtrlX ", " CtrlY ", " CtrlW ", " CtrlH "</span>"
			, ViewStrPos2 := _DP "<span lang='MS:'>" CtrlCAX ", " CtrlCAY ", " CtrlCAX2 ", " CtrlCAY2 "</span>" _DP "<span lang='MS:'>" CtrlCAX ", " CtrlCAY ", " CtrlW ", " CtrlH "</span>"
			. _DP "<span lang='MS:'>" CtrlSCX ", " CtrlSCY ", " CtrlSCX2 ", " CtrlSCY2 "</span>" _DP "<span lang='MS:'>" CtrlSCX ", " CtrlSCY ", " CtrlW ", " CtrlH "</span>" 	
	...
	If ControlNN !=
	{
		...
		WithRespectControl := _DP "<span lang='MS:'>" Round(rmCtrlX / CtrlW, 4) ", " Round(rmCtrlY / CtrlH, 4) "</span>"
	}
	Else
		rmCtrlX := rmCtrlY := ""
	...
	If UseUIA 
	{
		...
		. _PRE1 "<div " bc "><span class='param' lang='MS:N'>PID:</span>  <span lang='MS:'>" UIAPID "</span>" 
		
		. (UIAHWND ? _DP
		. "<span class='param' lang='MS:N'>HWND:</span>  <span lang='MS:'>" Format("0x{:x}", UIAHWND) "</span>"
		. _DP "<span class='param' lang='MS:N'>ControlClass:</span>  <span lang='MS:'>" TransformHTML(UIAElement.CurrentClassName) "</span>" : _DP "HWND undefined")
		. _DN
		. (CurrentControlTypeIndex != "" ? ""
		. "<span class='param' lang='MS:N'>ControlType:</span>  <span lang='MS:'>" CurrentControlTypeName "</span>"
		. _DP " <span lang='MS:'>" CurrentControlTypeIndex "</span>" : "ControlType undefined")
		
		. (CurrentAutomationId != "" ? _DP
		. "<span class='param' lang='MS:N'>AutomationId:</span>  <span lang='MS:'>" CurrentAutomationId "</span>" : _DP "AutomationId undefined")
		
		. (CurrentLocalizedControlType != "" ? _DP
		. "<span class='param' lang='MS:N'>LocalizedControlType:</span>  <span lang='MS:'>" CurrentLocalizedControlType "</span>" : _DP "LocalizedControlType undefined")
		
		. _DN "<span class='param' lang='MS:N'>ProcessName:</span>  <span lang='MS:'>" TransformHTML(UIAProcessName) "</span>"
		. _DP "<span class='param' lang='MS:N'>ProcessPath:</span>  <span lang='MS:'>" TransformHTML(UIAProcessPath) "</span></div>" _PRE2
	}
	...
	If ((S_CaretX := A_CaretX) != "")
		CaretPosStr = <span class='param'>Caret:</span>  <span lang='MS:'>x%S_CaretX% y%A_CaretY%</span>
	Else 
...
HTML_Control:
	If ControlID
	{
		...
		Relativescreen = <span class='param'>Relative screen:</span>  <span lang='MS:'>x%CtrlSCX% y%CtrlSCY%</span>%_DP%<span lang='MS:'>x&sup2;%CtrlSCX2% y&sup2;%CtrlSCY2%</span>
		
		HTML_ControlExist =
		( Ltrim
		%_T1% id='__Control'> ( Control ) </span>%_BT1% id='flash_control'> flash %_BT2%%_ButWindow_Detective%%_T2%
		%_PRE1%<span class='param'>ClassNN:</span>  <span lang='MS:'>%ControlNN%</span>%_DP%<span class='param'>Class:</span>  <span lang='MS:'>%CtrlClass%</span>
		%_BP1% id='set_button_pos'>Pos:%_BP2%  <span lang='MS:'>x%CtrlX% y%CtrlY%</span>%_DP%<span lang='MS:'>x&sup2;%CtrlX2% y&sup2;%CtrlY2%</span>%_DP%%_BP1% id='set_button_pos'>Size:%_BP2%  <span lang='MS:'>w%CtrlW% h%CtrlH%</span>%ViewStrPos1%
		<span class='param'>Relative client area:</span>  <span lang='MS:'>x%CtrlCAX% y%CtrlCAY%</span>%_DP%<span lang='MS:'>x&sup2;%CtrlCAX2% y&sup2;%CtrlCAY2%</span>%_DP%%Relativescreen%%ViewStrPos2%
		%_BP1% id='set_pos'>Mouse relative control:%_BP2%  <span lang='MS:'>x%rmCtrlX% y%rmCtrlY%</span>%WithRespectControl%
		<span class='param'>HWND:</span>  <span lang='MS:'>%ControlID%</span>%_DP%%_BB1% id='control_show_hide'> show / hide %_BB2%%_DP%%_BB1% id='control_destroy'> destroy %_BB2%%_DP%%_BP1% id='control_path'> Get path %_BP2%<span id='control_path_error'></span>%_ParentControl%
		<span class='param'>Style:</span>  <span id='c_Style' lang='MS:'>%CtrlStyle%</span>%_DP%<span class='param'>ExStyle:</span>  <span id='c_ExStyle' lang='MS:'>%CtrlExStyle%</span>%ButtonStyle_%%_PRE2%
	...
	%_PRE1%%_BP1% id='set_pos'>Screen:%_BP2%  <span lang='MS:'>x%MXS% y%MYS%</span>%_DP%%_BP1% id='set_pos'>Window:%_BP2%  <span lang='MS:'>x%RWinX% y%RWinY%</span>%_DP%%_BP1% id='set_pos'>Client:%_BP2%  <span lang='MS:'>x%MXC% y%MYC%</span>%WithRespectWin%%WithRespectClient%
	<span class='param'>Relative active window:</span>  <span lang='MS:'>x%MXWA% y%MYWA%</span>%_DP%<span class='param'>class</span> <span lang='MS:'>%WinClass_A%</span> <span class='param'>exe</span> <span lang='MS:'>%ProcessName_A%</span> <span class='param'>hwnd</span> <span lang='MS:'>%HWND_A%</span>%_PRE2%
	%_T1% id='__PixelColor'> ( PixelColor ) </span>%_T2%
	%_PRE1%<span class='param'>RGB: </span> <span lang='MS:'>%ColorRGB%</span>%_DP%#<span lang='MS:'>%sColorRGB%</span>%_DP%<span class='param'>BGR: </span> <span lang='MS:'>%ColorBGR%</span>%_DP%#<span lang='MS:'>%sColorBGR%</span>%_DP%<span class='param'>Invert RGB: </span> <span lang='MS:'>0x%sInvert_RGB%</span>%_DP%#<span lang='MS:'>%sInvert_RGB%</span>%_DP%<span style='background-color: #%sInvert_RGB%'>          </span>%_PRE2%
	%_T1% id='__Window'> ( Window ) </span>%_BT1% id='flash_ctrl_window'> flash %_BT2%%_T2%
	%_PRE1%<span><span class='param' lang='MS:S'>ahk_class</span> <span lang='MS:'>%WinClass%</span></span> <span><span class='param' lang='MS:S'>ahk_exe</span> <span lang='MS:'>%ProcessName%</span></span> <span><span class='param' lang='MS:S'>ahk_id</span> <span lang='MS:'>%WinID%</span></span> <span><span class='param' lang='MS:S'>ahk_pid</span> <span lang='MS:'>%WinPID%</span></span>
	<span class='param'>Cursor:</span>  <span lang='MS:'>%A_Cursor%</span>%_DP%%CaretPosStr%%_DP%<span class='param'>Client area:</span>  <span lang='MS:'>x%caX% y%caY% w%caW% h%caH%</span>
	%_BP1% id='set_button_focus_ctrl'>Focus control:%_BP2%  <span lang='MS:'>%CtrlFocus%</span>%_PRE2%
	%HTML_ControlExist%
...
GetMenuText(hMenu, child = 0)
{
	Loop, % DllCall("GetMenuItemCount", "Ptr", hMenu)
	{
		...
		IdItem := "<span class='param menuitemid' style='display: " (!MenuIdView ? "none" : "inline") ";;'>`t`t`t<span lang='MS:'>" idn "</span></span>"
		isSubMenu := (idn = -1) && (hSubMenu := DllCall("GetSubMenu", "Ptr", hMenu, "int", idx)) ? 1 : 0
		If isSubMenu
			sContents .= AddTab(child) "<span class='param'>" idx + 1 ":  </span><span lang='MS:' class='titleparam'>" sString "</span>" IdItem "`n"
		Else If (sString = "")
			sContents .= AddTab(child) "<span class='param'>" idx + 1 ":  &#8212; &#8212; &#8212; &#8212; &#8212; &#8212; &#8212;</span>" IdItem "`n"
		Else
			sContents .= AddTab(child) "<span class='param'>" idx + 1 ":  </span><span lang='MS:'>" sString "</span>" IdItem "`n"
		If isSubMenu
...
GetInfo_SysListView(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Row count:</span> <span lang='MS:'>" RowCount "</span>" _DP
			. "<span class='param' lang='MS:N'>Column count:</span> <span lang='MS:'>" ColCount "</span>`n"
			. "<span class='param' lang='MS:N'>Selected count:</span> <span lang='MS:'>" SelectedCount "</span>" _DP
			. "<span class='param' lang='MS:N'>Focused row:</span> <span lang='MS:'>" FocusedCount "</span>" _PRE2
...
GetInfo_SysTreeView(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Item count:</span> <span lang='MS:'>" ItemCount "</span>"
}
...
GetInfo_ComboBox(hwnd, ListBox = 0) { 
	...
	Return	"<span class='param' lang='MS:N'>Row count:</span> <span lang='MS:'>" RowCount "</span>" _DP
			. "<span class='param' lang='MS:N'>Row selected:</span> <span lang='MS:'>" SelPos "</span>" _PRE2
...
GetInfo_Edit(hwnd) { 
	...
	Return GetInfo_Scintilla(hwnd) "`n<span class='param' lang='MS:N'>FontSize:</span> <span lang='MS:'>" FSize "</span>" _DP "<span class='param' lang='MS:N'>FontName:</span> <span lang='MS:'>" FName "</span>"
		. "`n<span class='param' lang='MS:N'>DlgCtrlID:</span> <span lang='MS:'>" DllCall("GetDlgCtrlID", Ptr, hwnd) "</span>"
}
...
GetInfo_Scintilla(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Row count:</span> <span lang='MS:'>" LineCount "</span>" _DP
			. "<span class='param' lang='MS:N'>Selected length:</span> <span lang='MS:'>" StrLen(Selected) "</span>"
			. "`n<span class='param' lang='MS:N'>Current row:</span> <span lang='MS:'>" CurrentLine "</span>" _DP
			. "<span class='param' lang='MS:N'>Current column:</span> <span lang='MS:'>" CurrentCol "</span>"
			. "`n<span class='param' lang='MS:N'>Current select:</span> <span lang='MS:'>" EM_GETSEL "</span>" _DP
			. "<span class='param' lang='MS:N'>First visible line:</span> <span lang='MS:'>" EM_GETFIRSTVISIBLELINE "</span>"
}
...
GetInfo_msctls_progress(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Level:</span> <span lang='MS:'>" PBM_GETPOS "</span>" _DP
			. "<span class='param'>Range:  </span><span class='param' lang='MS:N'>Min: </span><span lang='MS:'>" PBM_GETRANGEMIN "</span>"
			. "  <span class='param' lang='MS:N'>Max:</span> <span lang='MS:'>" PBM_GETRANGEMAX "</span>"
}
...
GetInfo_msctls_trackbar(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Level:</span> <span lang='MS:'>" TBM_GETPOS "</span>" _DP
			. "<span class='param'>Invert style:</span>" TBS_REVERSED
			. "`n<span class='param'>Range:  </span><span class='param' lang='MS:N'>Min: </span><span lang='MS:'>" TBM_GETRANGEMIN "</span>" _DP
			. "<span class='param' lang='MS:N'>Max:</span> <span lang='MS:'>" TBM_GETRANGEMAX "</span>"
}
...
GetInfo_msctls_updown(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Level:</span> <span lang='MS:'>" UDM_GETPOS32 "</span>" _DP
			. "<span class='param'>Range:  </span><span class='param' lang='MS:N'>Min: </span><span lang='MS:'>" UDM_GETRANGE >> 16 "</span>"
			. "  <span class='param' lang='MS:N'>Max: </span><span lang='MS:'>" UDM_GETRANGE & 0xFFFF "</span>"
}
...
GetInfo_SysTabControl(hwnd) { 
	...
	Return	"<span class='param' lang='MS:N'>Item count:</span> <span lang='MS:'>" TCM_GETITEMCOUNT "</span>" _DP
			. "<span class='param' lang='MS:N'>Row count:</span> <span lang='MS:'>" TCM_GETROWCOUNT "</span>" _DP
			. "<span class='param' lang='MS:N'>Selected item:</span> <span lang='MS:'>" SelTab "</span>"
}

GetInfo_ToolbarWindow(hwnd) { 
	SendMessage, 0x0418,,,, ahk_id %hwnd%		;;  TB_BUTTONCOUNT
	BUTTONCOUNT := ErrorLevel
	Return	"<span class='param' lang='MS:N'>Button count:</span> <span lang='MS:'>" BUTTONCOUNT "</span>"
}
...
GetInfo_InternetExplorer_Server(hwnd) {
	...
	If (Tag = "IFRAME" || Tag = "FRAME")
	{
		...
		If ((Var := WB2.LocationName) != "")
			Frame .= "`n<span class='param' lang='MS:N'>Title:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
		If ((Var := WB2.LocationURL) != "")
			Frame .= "`n<span class='param' lang='MS:N'>URL:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
		If (iFrame.length)
			Frame .= "`n<span class='param' lang='MS:N'>Count frames:  </span><span lang='MS:'>" TransformHTML(iFrame.length) "</span>"
		If (Tag != "")
			Frame .= "`n<span class='param' lang='MS:N'>TagName:  </span><span lang='MS:'>" TransformHTML(Tag) "</span>"
		If ((Var := pelt.id) != "")
			Frame .= "`n<span class='param' lang='MS:N'>ID:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
		If ((Var := pelt.className) != "")
			Frame .= "`n<span class='param' lang='MS:N'>Class:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
		If ((Var := pelt.sourceIndex) != "")
			Frame .= "`n<span class='param' lang='MS:N'>Index:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
		If ((Var := pelt.name) != "")
			Frame .= "`n<span class='param' lang='MS:N'>Name:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
	...
	If ((Location := WB2.LocationName) != "")
		Topic .= "<span class='param' lang='MS:N'>Title:  </span><span lang='MS:'>"  TransformHTML(Location) "</span>`n"
	If ((URL := WB2.LocationURL) != "")
		Topic .= "<span class='param' lang='MS:N'>URL:  </span><span lang='MS:'>"  TransformHTML(URL) "</span>"
	If Topic !=
		Topic := _PRE1 Topic _PRE2

	If ((Var := pelt.id) != "")
		Info .= "`n<span class='param' lang='MS:N'>ID:  </span><span lang='MS:'>" Var "</span>"
	If ((Var := pelt.className) != "")
		Info .= "`n<span class='param' lang='MS:N'>Class:  </span><span lang='MS:'>" Var "</span>"
	If ((Var := pelt.sourceIndex) != "")
		Info .= "`n<span class='param' lang='MS:N'>Index:  </span><span lang='MS:'>" Var "</span>"
	If ((Var := pelt.name) != "")
		Info .= "`n<span class='param' lang='MS:N'>Name:  </span><span lang='MS:'>" TransformHTML(Var) "</span>"
	...
	If (pelt.TagName)
		Info := _T1 " id='P__Tag_name' lang='MS:N'> ( Tag name: <span lang='MS:' style='color: #" ColorFont ";'>"
		. pelt.TagName "</span>" (Frame ? " - (in frame)" : "") " ) </span>" _T2
		. _PRE1  "<span class='param'>Pos: </span><span lang='MS:'>x" Round(x1) " y" Round(y1) "</span>"
		. _DP "<span lang='MS:'>x&sup2;" Round(x2) - 1 " y&sup2;" Round(y2) - 1 "</span>"
		. _DP "<span class='param'>Size: </span><span lang='MS:'>w" Round(x2 - x1) " h" Round(y2 - y1) "</span>" Info _PRE2
...
AccInfoUnderMouse(mx, my, wx, wy, cx, cy, caX, caY, WinID, ControlID) {
	...
	If child
		Var := "<span lang='MS:'>Simple Element</span>" _DP "<span class='param' lang='MS:N'>Id:  </span>" child
	Else If ChildCount
		Var := "<span lang='MS:'>Container</span>" _DP "<span class='param' lang='MS:N'>ChildCount:  </span>" ChildCount _DP "<span class='param' lang='MS:N'>Id:  </span>" child 
	Else
		Var := "<span lang='MS:'>Real Object</span>" _DP "<span class='param' lang='MS:N'>Id:  </span>" child    ;;  _DP "<span class='param' lang='MS:N'>Container child count:  </span>" ChildCount
	...
	code .= _T1 " id='P__Position_relative_Acc'" _T1P "> ( Position relative ) </span>" _T2 _PRE1 "<span class='param'>Screen: </span>"
		. "<span lang='MS:'>x" x " y" y "</span>"
		. _DP "<span lang='MS:'>x&sup2;" x + w - 1 " y&sup2;" y + h - 1 "</span>"
		. _DP "<span class='param'>Size: </span><span lang='MS:'>w" w " h" h "</span>"
		.  _DP  "<span class='param'>Mouse: </span><span lang='MS:'>x" mx - AccCoord[1] " y" my - AccCoord[2] "</span>`n"

		. "<span class='param'>Window: </span><span lang='MS:'>x" x - wx " y" y - wy "</span>"
		. _DP "<span lang='MS:'>x&sup2;" x - wx + w - 1 " y&sup2;" y - wy + h - 1 "</span>"
		
		. _DP "<span class='param'>Client: </span><span lang='MS:'>x" x - wx - caX " y" y - wy - caY "</span>"
		. _DP "<span lang='MS:'>x&sup2;" x - wx + w - 1 - caX " y&sup2;" y - wy + h - 1 - caY "</span>"
		 
		. (cx != "" ? _DP "<span class='param'>Control: </span><span lang='MS:'>x" (x - wx - cx) " y" (y - wy - cy) "</span>"
		. _DP "<span lang='MS:'>x&sup2;" (x - wx - cx) + w - 1 " y&sup2;" (y - wy - cy) + h - 1 "</span>" : "")  _PRE2
	...
	If ((Hwnd := AccWindowFromObject(pacc)) != ControlID && Hwnd != WinID) {   ;	можно Acc вместо pacc, then ComObjValue
		...
		. "<div style='background-color: #" HighLightBckg "'><span class='param' lang='MS:N'>HWND:</span>  <span lang='MS:'>" Format("0x{:x}", Hwnd) "</span>"
		. _DP "<span class='param' lang='MS:N'>Class:</span>  <span lang='MS:'>" TransformHTML(CtrlClass) "</span>"
		. _DP "<span class='param' lang='MS:N'>Exe:</span>  <span lang='MS:'>" TransformHTML(WinProcess) "</span>"
		. _DP "<span class='param' lang='MS:N'>PID:</span>  <span lang='MS:'>" WinPID "</span></div>" _PRE2
	}
	...
	If (strstyles != "")
		code .= _T1 " id='P__State_Acc'" _T1P "> ( State: <span lang='MS:' style='color: #" ColorFont ";'>" style "</span> ) </span>" _T2 _PRE1 strstyles _PRE2
		
	If ((Var := AccRole(Acc, child)) != "")
		code .= _T1 " id='P__Role_Acc'" _T1P "> ( Role ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>"
		. _DP "<span class='param' lang='MS:N'>code: </span><span lang='MS:'>" Acc.accRole(child) "</span>" _PRE2
		
	If (child &&(Var := AccRole(Acc)) != "")
		code .= _T1 " id='P__Role_parent_Acc'" _T1P "> ( Role - parent ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>"
		. _DP "<span class='param' lang='MS:N'>code: </span><span lang='MS:'>" Acc.accRole(0) "</span>" _PRE2
		
	If ((Var := Acc.accDefaultAction(child)) != "")
		code .= _T1 " id='P__Action_Acc'" _T1P "> ( Action ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>" 
		. _DP _BP1 " id='acc_DoDefaultAction'> Execute " _BP2 _PRE2
		
	If ((Var := Acc.accSelection) > 0)
		code .= _T1 " id='P__Selection_parent_Acc'" _T1P "> ( Selection - parent ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>" _PRE2
	
	If ((Var := Acc.accDescription(child)) != "")
		code .= _T1 " id='P__Description_Acc'" _T1P "> ( Description ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>" _PRE2
	
	If ((Var := Acc.accKeyboardShortCut(child)) != "")
		code .= _T1 " id='P__ShortCut_Acc'" _T1P "> ( ShortCut ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>" _PRE2
	
	If ((Var := Acc.accHelp(child)) != "")
		code .= _T1 " id='P__Help_Acc'" _T1P "> ( Help ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>" _PRE2
	
	If ((Var := Acc.AccHelpTopic(child)))
		code .= _T1 " id='P__HelpTopic_Acc'" _T1P "> ( HelpTopic ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(Var) "</span>" _PRE2
	...
	If (role != "" && irole != "")
		, code .= _T1 " id='P__Role_Acc'" _T1P "> ( Focus - Role ) </span>" _T2 _PRE1 "<span lang='MS:'>" TransformHTML(role) "</span>"
		. _DP "<span class='param' lang='MS:N'>code: </span><span lang='MS:'>" irole "</span>" _PRE2 
	Return code
} 
...
AccState(Acc, child, byref style, byref str, i := 1) {
	...
	If (style = 0)
		Return "", str := "<span class='param' lang='MS:'>" AccGetStateText(0) "</span>" _DP "<span lang='MS:'>" 0x00000000 "</span>`n"
	While (i <= style) {
		if (i & style)
			str .= "<span class='param' lang='MS:'>" AccGetStateText(i) "</span>" _DP "<span lang='MS:'>" Format("0x{1:08X}", i) "</span>`n"
		i <<= 1
	}
}
...
GetAccPath() { 
	...
	for k, v in arr
	{ 
		tree .= AddSpace(k - 1) "<span><span lang='MS:'>" v.Path "</span>" _DP  "<span lang='MS:'>" v.Hwnd "</span>" 
			. _DP "<span lang='MS:'>" v.WinClass "</span>" _DP  "<span lang='MS:'>" v.ProcessName "</span></span><span lang='MS:P'>     </span>"
...
Write_HotkeyHTML(K) {
	...
	If K.IsCode
		Comment := "<span class='param' lang='MS:SP'>    `;  """ KeyName """</span>"
	If (Hotkey != "")
		FComment := "<span class='param' lang='MS:SP'>    `;  """ (K.HK = "" ? K.TK : Mods KeyName) """</span>"

	If (LRMods != "")
	{
		LRMStr := "<span lang='MS:'>" LRMods KeyName "</span>"
		If (K.HK != "")
			LRPStr := "  " _DP "  <span><span lang='MS:'>" LRPref Hotkey "::</span><span class='param' lang='MS:SP'>    `;  """ LRMods KeyName """</span></span>"
	}
	If Prefix !=
	...
	If (DUMods != "") 
		LRSend := "  " _DP "  <span><span lang='MS:'>" SendMode  " <span lang='MS:'>" DUMods "</span></span>" Comment "</span>"
	If SCCode !=
		ThisKeySC := "   " _DP "   <span lang='MS:'>" VKCode "</span>   " _DP "   <span lang='MS:'>" SCCode "</span>   "
		. _DP "   <span lang='MS:' id='v_VKDHCode'>" VKCode_ "</span>   " _DP "   <span lang='MS:' id='v_SCDHCode'>" SCCode_ "</span>"
	Else
		ThisKeySC := "   " _DP "   <span lang='MS:' id='v_VKDHCode'>" VKCode_ "</span>"
	...
	%_PRE1%<br><span lang='MS:'>%Mods%%KeyName%</span>%NotPhysical%<br><br>%LRMStr%<br>%_PRE2%
	%_T1%> ( Command syntax ) </span>%_BT1% id='SendCode'> %SendCode% %_BT2%%_DB%%_BT1% id='SendMode'> %SendModeStr% %_BT2%%_T2%
	%_PRE1%<br><span><span lang='MS:'>%Prefix%%Hotkey%::</span>%FComment%</span>%LRPStr%
	<span lang='MS:P'>        </span>
	<span><span lang='MS:'>%SendMode% <span lang='MS:'>%Prefix%{%SendHotkey%}</span></span>%Comment%</span>%LRSend%
	<span lang='MS:P'>        </span>
	<span><span lang='MS:'>ControlSend, ahk_parent, <span lang='MS:'>%ControlSend%</span>, WinTitle</span>%Comment%</span>
	<span lang='MS:P'>        </span>
	<span><span lang='MS:'>GetKeyState("%SendHotkey%", "P")</span>%Comment%</span>   %_DP%   <span><span lang='MS:'>KeyWait, %SendHotkey%, D T0.5</span>%Comment%</span>
	<span lang='MS:P'>        </span>
	<span><span lang='MS:'>%HK2% & %HK1%::</span><span class='param' lang='MS:SP'>%HKComm1% & %HKComm2%</span></span>   %_DP%   <span><span lang='MS:'>%HK2%::%HK1%</span><span class='param' lang='MS:SP'>%HKComm1% &#8250 &#8250 %HKComm2%</span></span>
	<span lang='MS:P'>        </span>%_PRE2%
	...
	%_PRE1%<br><span lang='MS:'>%ThisKey%</span>   %_DP%   <span lang='MS:'>%VKCode%%SCCode%</span>%ThisKeySC%
...
QVK(key, value, VK = 1) {
	...
	If VK
		Return T _PRE1 Description1 "<span><span lang='MS:'>" key "</span><span class='param' lang='MS:SP'> := " value "</span></span><br>" _PRE2
	Return T _PRE1 Description2 "<span style='color: #C0C0C0'>" key "</span><br>" _PRE2
}

GetScanCode(id) { 
	If SC := GetKeySC(id) 
		Return _T1 "> ( GetKeySC ) </span>" _T2 "<br>" _PRE1 "<span lang='MS:'>" Format("0x{:03X}", SC) "</span><br>" _PRE2 
}
...
ChildToPath(hwnd) {
	...
	{  
		...
		tree .= AddSpace2(k - 1) "<span><span lang='MS:'>" v.Path "</span>" _DP  "<span lang='MS:'>" Hwnd "</span>" 
			. _DP "<span lang='MS:'>" WinClass "</span>" _DP  "<span lang='MS:'>" ProcessName "</span></span><span lang='MS:P'>     </span>"
...
FixIE() {
	...
	. "\FeatureControl\FEATURE_BROWSER_EMULATION", ver := 110001
	If A_IsCompiled
...
ClipPaste() {
 	If oMS.ELSel && (oMS.ELSel.OuterText != "" || MS_Cancel())
		oMS.ELSel.innerHTML := TransformHTML(Clipboard), oMS.ELSel.lang := "MS:"
...
GetCLSIDExplorer(hwnd) {
	for window in ComObjCreate("Shell.Application").Windows
		If (window.hwnd = hwnd)
			Return (CLSID := window.Document.Folder.Self.Path) ~= "^::\{" ? "`n<span class='param'>CLSID: </span><span lang='MS:'>" CLSID "</span>": ""
}
...
QStyleTitle(Title, Name, F, V) {
	If Name !=
		Return " ( " Title " - <span lang='MS:' style='color: #" ColorParam ";'>" Name "</span>: <span lang='MS:' style='color: #" ColorFont ";'>" Format("0x{:0" F "X}", V) "</span> ) " 
	Return " ( " Title ": <span lang='MS:' style='color: #" ColorFont ";'>" Format("0x{:0" F "X}", V) "</span> ) "  
}
QStyleRest(F, V) {
	Return "<span style='color: #" ColorDelimiter ";' lang='MS:'>" Format("0x{1:0" F "X}", V) "</span>`n"
}
QStyle(k, v, q = "") {
	Return "<span lang='MS:Q'>" k " := <span class='param' lang='MS:'>" v "</span></span>" . (q != "" ? _StIf q "</span>`n" : "`n")
}
...
GetStyle_SysPager(Style, hWnd)  {
	...
	If !(Style & oStyles.PGS_HORZ)
		Ret .= "<span lang='MS:'>PGS_VERT := <span class='param' lang='MS:'>0x0000   !(PGS_HORZ)</span></span>`n"
	For K, V In oStyles
		If ((Style & V) = V) && (1, Style -= V)
			Ret .= QStyle(K, V)
	IF Style
...
MS_Select(EL) { 
	...
	Else If InStr(EL.lang, ":N")
		oMS.ELSel := oDoc.all.item(EL.sourceIndex + 1)
	Else If InStr(EL.lang, ":P")
		oMS.ELSel := oDoc.all.item(EL.sourceIndex - 2).ParentElement
	Else
		oMS.ELSel := EL
	...
	If (InStr(EL.lang, ":SP") || InStr(EL.lang, ":Q") || InStr(EL.lang, ":P")) 
		oMS.ELSelChild :=  oMS.ELSel.lastChild           ; childNodes[1]   firstChild.ParentElement   lastChild
		, oMS.ELSelChild.style.color := "#FFFFFF"
	
	;; ToolTip % oMS.ELSelChild.OuterText "`n" EL.lang
}
...
<script type="text/javascript">
	...
	onmousedown = function() {  // строка 57 
...
//			return
	...
	onmouseup = function() {
		...
		// el.style.color = (el.lang != "pre" ? "#%ColorFont%" : "#%ColorParam%");
	...
	function ElementName (param) {
		try {
			el = document.querySelector("[lang=" + param + "]");
		} catch (e) {
			return
		}
		return el
	...
	ondblclick() {
		...
		Else If (ThisMode != "Hotkey" && (oevent.className = "title" || oevent.className = "con" || oevent.className = "hr" || oevent.className = "box"))  ;;	anchor
		{
			...
			Else If oevent.className = "hr"
				_text := oevent.parentElement.lastChild.firstChild.id, EL := oevent
			Else If oevent.className = "box"
				_text := oevent.firstChild.lastChild.firstChild.id, EL := oevent.firstChild.firstChild
...
ButtonClick(oevent) { 
	...
	{
		Loop % oDoc.getElementById("copy_sbtext").lang
	...
	{ 
		ControlClick, % oevent.lang, % "ahk_id" oOther.WinID, , , , Pos
	}
